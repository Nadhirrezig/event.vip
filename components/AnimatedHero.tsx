...
export default function VaporizeTextCycle({
  ...
  const isInView = useIsInView(wrapperRef as React.RefObject<HTMLElement>, pauseWhenHidden);
  ...
  const shouldAnimate = pauseWhenHidden ? isInView : true;
  ...
  // Start animation cycle when allowed
  useEffect(() => {
    if (!shouldAnimate) {
  ...
  }, [shouldAnimate, clearWaitTimeout]);

  // Animation loop - only run when allowed
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      ...
      switch (animationState) {
        ...
        case "fadingIn": {
          ...
          if (fadeOpacityRef.current >= 1 && !waitTimeoutRef.current) {
            setAnimationState((prev) => (prev === "waiting" ? prev : "waiting"));
 ...
 */
function useIsInView(ref: React.RefObject<HTMLElement>, enabled = true) {
